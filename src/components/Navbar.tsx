    ...
    <nav className="fixed top-0 left-0 w-full z-50 backdrop-blur-sm bg-black/50 shadow-md">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <Link
          ...
          duration={600}
          offset={-60}
          className="cursor-pointer text-saberTeal font-display text-2xl md:text-3xl font-bold drop-shadow-glow"
        >
          PAARTH
        </Link>
        ...
        {/* Mobile Menu Toggle */}
        <button
          className="md:hidden text-saberTeal text-2xl"
      ...
      {/* Mobile Menu */}
      {open && (
        <ul className="md:hidden px-6 pb-4 flex flex-col space-y-3 font-display text-starWhite bg-black/80 border-t border-saberTeal/20">
          {navLinks.map((link) => (
            <li key={link.name}>
              <Link
                to={link.to}
                ...
                spy={true}
                activeClass="text-saberTeal drop-shadow-glow"
                className="block cursor-pointer hover:text-saberTeal transition duration-200"
